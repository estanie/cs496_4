...
class DQN:
    ...
    def __init__(self, session, n_action):
        ...
        self.width = SEND_WIDTH * 2
        self.height = SEND_HEIGHT * 2
        # 게임 플레이결과를 저장할 메모리
        self.memory = deque()
        # 현재 게임판의 상태
        self.state = None
        ...
        self.input_X = tf.placeholder(tf.float32, [None, SEND_WIDTH * 2, SEND_HEIGHT * 2, self.STATE_LEN])
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            model = tf.layers.conv2d(self.input_X, 16, [4, 4], padding='same', activation=tf.nn.relu)
